...
- Реальный прогон нагрузочного теста выполнен напрямую против собранных бинарников сервисов и временного embedded-PostgreSQL (без установки в систему, без Docker) — то есть тот же код, реальные вызовы, реальная БД, просто не через `docker-compose`.
